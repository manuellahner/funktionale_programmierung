  ...
  case winningPlayer state of
    ...
    Nothing -> let moves = validMoves state in
      if null moves then putStrLn "Game ends in draw."
        ...
        putStr $ "Choose one of " ++ show moves ++ "or s(afe): "
...
newgame st  = do
  ...
  if(moveStr=="y") then do
    ...
    do
      putStr"Bis zum nächsten mal"


loadgame=do 
  s <- readFile "connect4.txt"
  game (stringToState s) (reversepc s)


safegame s= do
  writeFile  "connect4.txt" (showState s)